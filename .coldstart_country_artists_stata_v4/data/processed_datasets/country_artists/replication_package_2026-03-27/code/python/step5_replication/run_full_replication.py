...
def copy_python_tree(project_root: Path, package_root: Path) -> None:
    mapping = {
        "execution/step4_country_artists": {
            "dst": "code/python/step4_country_artists",
            "files": ["build_country_artists_dataset.py"],
            "fallback": package_root / "code" / "python" / "step4_country_artists",
        },
        "execution/step5_replication": {
            "dst": "code/python/step5_replication",
            "files": ["run_full_replication.py"],
            "fallback": package_root / "code" / "python" / "step5_replication",
        },
    }
    for rel_src, config in mapping.items():
        src_dir = project_root / rel_src
        if not src_dir.exists():
            src_dir = config["fallback"]
        dst_dir = package_root / config["dst"]
...
def copy_replication_assets(project_root: Path, package_root: Path) -> None:
    assets = [
        (
            project_root / "execution" / "step5_replication" / "run_country_artists_replication.do",
