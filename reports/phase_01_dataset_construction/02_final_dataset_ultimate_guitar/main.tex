...
\midrule
Rows missing \texttt{bpm} & 37,939 & 21,679 \\
Rows missing \texttt{genre} & 2,398 & 109 \\
Rows with official genre source (\texttt{genre\_is\_official = 1}) & 0 & 20,505 \\
Rows missing \texttt{birth\_state} in the final song file & 273 & 147 \\
Rows missing or \texttt{Unknown} in \texttt{us\_macro\_region} & 2,474 & 2,349 \\
\bottomrule
\end{tabular}
\end{table}
...
Genre source & Rows \\
\midrule
\texttt{apple\_music\_track} & 17,583 \\
\texttt{deezer\_album\_genre} & 2,833 \\
\texttt{discogs\_release} & 89 \\
\texttt{artist\_metadata\_fallback} & 812 \\
\texttt{ug\_selected} & 22,632 \\
Still empty & 109 \\
\midrule
BPM source & Rows \\
\midrule
\texttt{apple\_preview\_estimated} & 14,785 \\
\texttt{deezer\_track} & 1,311 \\
\texttt{deezer\_preview\_estimated} & 164 \\
...
chunked strategy on 2026-04-20: small BPM-only batches are now run through the
same hierarchical targeted pass, with preview estimation enabled but exact
iTunes and Discogs queries automatically disabled for the rest of a run once
they start returning \texttt{403} or \texttt{429}. The retained result is
therefore the current hierarchical backfill plus chunked residual recovery, not
a claim that every residual BPM gap has already been solved.
