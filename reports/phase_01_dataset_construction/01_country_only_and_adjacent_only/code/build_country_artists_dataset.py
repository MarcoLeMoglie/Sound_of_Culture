...
def resolve_project_root() -> Path:
    env_root = os.environ.get("SOC_PROJECT_ROOT")
    if env_root:
        return Path(env_root).resolve()
    candidate = SCRIPT_PATH.parents[2]
    if (candidate / "directives" / "costruzione dataset artisti country" / "country_artists_schema.csv").exists():
        return candidate
    return Path.cwd().resolve()


def resolve_schema_path(project_root: Path) -> Path:
    env_schema = os.environ.get("SOC_COUNTRY_SCHEMA_PATH")
    if env_schema:
        return Path(env_schema).resolve()
    repo_schema = project_root / "directives" / "costruzione dataset artisti country" / "country_artists_schema.csv"
    if repo_schema.exists():
        return repo_schema
    for parent in SCRIPT_PATH.parents:
        package_schema = parent / "docs" / "directives_snapshot" / "country_artists_schema.csv"
        if package_schema.exists():
            return package_schema
    raise FileNotFoundError("country_artists_schema.csv not found in project directives or package snapshot")
