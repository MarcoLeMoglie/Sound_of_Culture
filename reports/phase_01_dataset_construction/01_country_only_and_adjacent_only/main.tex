...
\texttt{execution/phase\_01\_dataset\_construction/backfill\_country\_only\_final\_artist\_metadata.py},
...
Metric & Count \\
\midrule
Rows still missing \texttt{birth\_state} & 147 \\
Rows still coded as missing or \texttt{Unknown} in \texttt{us\_macro\_region} & 2,349 \\
Rows still missing \texttt{birth\_country} & 121 \\
Unique artists still missing \texttt{birth\_state} & 40 \\
Unique artists still missing or \texttt{Unknown} in \texttt{us\_macro\_region} & 123 \\
Unique artists still missing \texttt{birth\_country} & 35 \\
US artists still missing \texttt{birth\_state} & 1 \\
\bottomrule
\end{tabular}
\end{table}

The last row matters most for the substantive geography problem. After the
retained repair pass and one additional web-confirmed update on 2026-04-20,
only one artist that is currently coded as US-origin still lacks a state
assignment in the final song-level file: \texttt{The Wreckers}. Most remaining
\texttt{Unknown} macro-region cases are now non-US artists, for whom a US
macro-region is conceptually unavailable rather than merely missing.
...
\midrule
\endhead
\texttt{code/build\_country\_artists\_dataset.py} & 2123--2258; 2329--2353; 2393--2439; 2442 onward & Build the base country-artist datasets, deduplicate, and export the first QC layer. \\
\texttt{code/build\_extended\_artist\_universe.py} & 571--583; 586--607; 610--691; 694--845; 869--945; 948 onward & Build the country core, expand to adjacent-only, and write the final universe QC and methodological notes. \\
\texttt{code/enrich\_artist\_universe\_missing\_metadata.py} & 2062--2085; 2119 onward & Recover missing key metadata directly on the operational country-only file used downstream. \\
\texttt{code/run\_artist\_universe\_replication.py} & 6--20; 57--65; 68--75; 87--108; 111--121; 128--145 & Restore or refresh the exact artist-universe replication package. \\
\bottomrule
\end{longtable}
