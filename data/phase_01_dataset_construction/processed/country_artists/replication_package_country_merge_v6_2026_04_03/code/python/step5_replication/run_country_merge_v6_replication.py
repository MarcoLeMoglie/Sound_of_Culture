...
def ensure_structure(package_root: Path) -> None:
    for path in [
        package_root,
        package_root / "code" / "python" / "step2_digitalize",
...
def copy_python_tree(project_root: Path, package_root: Path) -> None:
    ...
    for rel_dir, files in mapping.items():
        ...
        dst_dir = package_root / "code" / "python" / Path(rel_dir).name
...
def copy_assets(project_root: Path, package_root: Path) -> None:
    mapping = [
        (
            project_root / "execution" / "step5_replication" / "run_country_merge_v6_replication.py",
            package_root / "code" / "python" / "step5_replication" / "run_full_replication.py",
        ),
        (
            project_root / "execution" / "step5_replication" / "run_country_merge_v6_replication.do",
...
def script_path_for(project_root: Path, package_root: Path, script_name: str) -> str:
    ...
    packaged_script = package_root / "code" / "python" / "step2_digitalize" / script_name
