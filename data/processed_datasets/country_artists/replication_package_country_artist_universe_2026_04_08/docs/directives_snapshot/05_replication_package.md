...
10. Per i package snapshot-based, testare sia un cold start Python sia un cold start Stata in workspace isolato e verificare i conteggi finali dei file replicati.
11. Mantenere un launcher Stata complessivo che faccia partire in sequenza i package richiesti dall'utente.
...
- Per il package `artist-universe`, la replica canonica e` snapshot-based: il cold start deve ricreare esattamente `artist_universe_country_only`, `artist_universe_adjacent_only` e `artist_universe_country_plus_adjacent` in `csv` e `dta`.
...
8. `python3 execution/step5_replication/run_artist_universe_replication.py > log/run_artist_universe_replication.log`
